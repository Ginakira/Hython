/************************************************************
    ...
    Created Time: 2020/09/02 10:17:59
************************************************************/
...
#define BOP(op)                           \
    (*MasterChainSingle::run(tree[0], p)) \
        op(*MasterChainSingle::run(tree[1], p))
    ...
    switch (tree.type()) {
        case INT: {
            ...
            if (s[0] == '~') {
                ...
                return std::make_shared<IntValue>(s.c_str());
            }
        }
        case ID: {
...
SIValue ControlNoBlockMaster::run(ASTNode &tree,
